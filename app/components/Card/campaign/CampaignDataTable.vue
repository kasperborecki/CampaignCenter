<template>
  <div class="overflow-hidden rounded-2xl border border-white/6 bg-[#151821] shadow-[0_18px_45px_-30px_rgba(0,0,0,0.95)]">
    <article
    ...
    >
      ...
      <div class="min-w-0">
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2 text-xs font-medium text-slate-400">
          ...
          <span class="flex h-6 items-center rounded-lg bg-[#090c13] px-2">
            ...
            {{ campaign.budget.hidden ? t('campaignPage.hidden') : `${campaign.budget.amount} ${campaign.budget.currency}` }}
          </span>
          <span class="flex h-6 items-center rounded-lg bg-[#090c13] px-2">
      ...
      <div>
        ...
        <p class="mt-1 text-base font-semibold text-white">{{ campaign.creators.current }} / {{ campaign.creators.total }}</p>
      </div>

      <div>
        <p class="text-xs text-slate-400">{{ t('campaignPage.tasks') }}</p>
        <p class="mt-1 text-base font-semibold text-white">{{ campaign.tasks.current }} / {{ campaign.tasks.total }}</p>
      </div>
    ...
    <div class="flex items-center justify-between border-t border-white/5 bg-[#151821] px-4 py-3 sm:px-6">
      ...
      <nav aria-label="Pagination" class="isolate inline-flex gap-2 rounded-md">
        ...
        <Button
          ...
          :label="String(item)"
          :disabled="item === '...'"
          class="h-9 min-w-9 rounded-md px-3 text-sm font-semibold"
